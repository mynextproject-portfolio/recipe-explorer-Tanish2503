...
def test_create_recipe_missing_required_field_returns_422(client, clean_storage, sample_recipe_data):
...
def test_create_recipe_blank_title_returns_422(client, clean_storage, sample_recipe_data):
...
def test_create_recipe_blank_only_ingredients_returns_422(client, clean_storage, sample_recipe_data):
...
def test_create_recipe_too_many_ingredients_returns_422(client, clean_storage, sample_recipe_data):
    payload = dict(sample_recipe_data, ingredients=[f"ingredient {i}" for i in range(51)])
    response = client.post("/api/recipes", json=payload)
    assert response.status_code == 422
    assert any(err["field"] == "ingredients" for err in response.json()["errors"])


def test_create_recipe_missing_ingredients_key_returns_422(client, clean_storage, sample_recipe_data):
    payload = dict(sample_recipe_data)
...
def test_update_recipe_invalid_payload_returns_422(client, clean_storage, sample_recipe_data):
    create_response = client.post("/api/recipes", json=sample_recipe_data)
    recipe_id = create_response.json()["id"]

    response = client.put(f"/api/recipes/{recipe_id}", json={**sample_recipe_data, "title": ""})
    assert response.status_code == 422
...
def test_update_nonexistent_recipe_returns_404(client, clean_storage, sample_recipe_data):
...
def test_delete_existing_recipe_then_get_returns_404(client, clean_storage, sample_recipe_data):
    create_response = client.post("/api/recipes", json=sample_recipe_data)
    recipe_id = create_response.json()["id"]
...
def test_import_reports_skipped_invalid_entries(client, clean_storage, sample_recipe_data):
